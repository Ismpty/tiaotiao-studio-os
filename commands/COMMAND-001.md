# COMMAND-001 — Run Little Security Guard Production（运行小保安和他的古人朋友们完整生产流程）
...
Run Little Security Guard Production 是 TiaoTiao Studio OS 的默认完整生产启动命令。

它用于一键启动《小保安和他的古人朋友们》从项目设定、文物朋友身份卡、分镜、提示词、摄影、脚本、剪辑到发布规划的完整生产流程。
...
Run Little Security Guard Production is the default full production operating command in TiaoTiao Studio OS.

It starts the complete Little Security Guard and His Ancient Friends production process from project setup to relic-friend identity, storyboard, prompt, cinematography, script, editing and publishing planning.
...
```text
[ ] Jump 是否保持真实小狗本体形态
[ ] Jump 是否穿 AI 博物馆小保安服装或小狗服装
[ ] Jump 是否保持小保安身份
[ ] 文物朋友是否符合 RELIC-001
[ ] 文物朋友是否没有变成现代网红、恐怖鬼魂或欧美怪物
[ ] Jump 外观是否参考 ASSET-001
[ ] 世界观是否符合 WORLD-001
[ ] 风格是否符合 STYLE-001
[ ] 色彩是否符合 COLOR-001
[ ] 情绪是否符合 EMOTION-001
[ ] 故事是否符合 RELIC-001 / STORY-001
...
- 必须读取 RELIC-001 和 SUBJECT-001
- 必须读取视觉参数、场景调度、镜头一致性和转场知识节点
- 必须调用 6 个 Agents
- 必须输出完整生产包
- 必须执行最终一致性检查
...
- Little Security Guard and His Ancient Friends production
...
|---|---|---|
| 2.0 | 2026 | Pivoted default full production command to Little Security Guard and His Ancient Friends |
